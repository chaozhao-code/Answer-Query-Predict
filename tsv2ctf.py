...
def populate_dicts(files):
    ...
    for f in files:
        with open('/root/BiDAF/data/%s.tsv' % f, 'r', encoding='utf-8') as input_file:
    ...
    with open('/root/BiDAF/data/big_data/glove.840B.300d.txt', encoding='utf-8') as f:
        for line in f:
...
if __name__=='__main__':
    try:
        known, vocab, chars = pickle.load(open('/root/BiDAF/data/big_data/vocabs.pkl', 'rb'))
    except:
        known, vocab, chars = populate_dicts(tsvs)
        f = open('/root/BiDAF/data/big_data/vocabs.pkl', 'wb')
        pickle.dump((known, vocab, chars), f)
        f.close()

    for tsv in tsvs:
        with open('/root/BiDAF/data/big_data/%s.tsv' % tsv, 'r', encoding='utf-8') as f:
            with open('/root/BiDAF/data/big_data/%s.ctf' % tsv, 'w', encoding='utf-8') as g:
