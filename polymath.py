...
class PolyMath:
    def __init__(self, config_file):
        ...
        self.abs_path = '/root/BiDAF/data/big_data'
    ...
    def embed(self):
        ...
        with open(os.path.join(self.abs_path, 'glove.840B.300d.txt'), encoding='utf-8') as f:
            for line in f:
                parts = line.split(' ')
                word = parts[0].lower()
                if word in self.vocab:
                    #print("parts[1:] len: %d" % len(parts[1:]))
                    #print(npglove.shape)
                    try:
    ...
    def input_layer(self,cgw,cnw,cc,qgw,qnw,qc):
        ...
        processed = OptimizedRnnStack(self.hidden_dim, recurrent_op='gru', bidirectional=True, use_cudnn=self.use_cudnn, name='input_rnn')(highway_drop)
    ...
    def modeling_layer(self, attention_context):
        ...
        mod_context = C.layers.Sequential([
            C.layers.Dropout(self.dropout),
            OptimizedRnnStack(self.hidden_dim, recurrent_op='gru', bidirectional=True, use_cudnn=self.use_cudnn, name='model_rnn0'),
            C.layers.Dropout(self.dropout),
            OptimizedRnnStack(self.hidden_dim, recurrent_op='gru', bidirectional=True, use_cudnn=self.use_cudnn, name='model_rnn1')])(att_context)
    ...
    def output_layer(self, attention_context, modeling_context):
        ...
        m2 = OptimizedRnnStack(self.hidden_dim, recurrent_op='gru', bidirectional=True, use_cudnn=self.use_cudnn, name='output_rnn')(end_input)
